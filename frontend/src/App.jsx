        ...
        >
            {isEditing ? (
                ...
                <div className="flex items-start gap-2">
                    ...
                    <p className="text-gray-700 flex-1 leading-relaxed break-words">{note}</p>
...
function StatisticsPage({ clusters, onBack }) {
    const totalNotes = clusters.reduce((sum, c) => sum + c.notes.length, 0);
    
    const orangeShades = [
        '#fb923c', '#f97316', '#ea580c', '#c2410c', '#fdba74',
        '#fed7aa', '#9a3412', '#ffedd5', '#7c2d12', '#fbbf24',
    ];

    return (
        <div className="min-h-screen p-8" style={{ backgroundColor: 'var(--custom-app-bg)' }}>
            <div className="max-w-6xl mx-auto">
                {/* Header */}
                <div className="relative flex justify-center mb-8">
                    <button
                        onClick={onBack}
                        className="absolute left-0 flex items-center gap-2 px-4 py-2 bg-white border-2 border-orange-500 text-orange-600 rounded-full hover:bg-orange-50 transition-all shadow-lg"
                    >
                    ...
                    <h1 className="text-4xl font-bold text-orange-600" style={{ fontFamily: '"League Spartan", sans-serif' }}>
                        Analytics
                    </h1>
                </div>
                ...
                {clusters.length > 0 ? (
                    <>
                        {/* Pie Chart */}
                        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 border-2 border-orange-200 flex flex-col items-center">
                            <h3 className="text-2xl font-bold text-gray-800 mb-6 text-center" style={{ fontFamily: '"League Spartan", sans-serif' }}>
                                ...
                                <PieChart>
                                    <Pie
                                        data={clusters.map(cluster => ({
                                            ...
                                            value: cluster.notes.length,
                                            percentage: ((cluster.notes.length / totalNotes) * 100).toFixed(1)
                                        }))}
                                        cx="50%"
                                        cy="50%"
                                        labelLine={true}
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                            <div className="bg-gradient-to-br from-orange-400 to-orange-500 p-6 rounded-2xl text-center shadow-lg transform hover:scale-105 transition-transform">
                                <p className="text-4xl font-bold text-white mb-2">{clusters.length}</p>
                                <p className="text-sm text-orange-50 font-semibold">Total Clusters</p>
                            </div>
                            <div className="bg-gradient-to-br from-orange-500 to-orange-600 p-6 rounded-2xl text-center shadow-lg transform hover:scale-105 transition-transform">
                                <p className="text-4xl font-bold text-white mb-2">{totalNotes}</p>
                                <p className="text-sm text-orange-50 font-semibold">Total Notes</p>
                            </div>
                            <div className="bg-gradient-to-br from-orange-600 to-orange-700 p-6 rounded-2xl text-center shadow-lg transform hover:scale-105 transition-transform">
                                <p className="text-4xl font-bold text-white mb-2">{Math.max(...clusters.map(c => c.notes.length))}</p>
                                <p className="text-sm text-orange-50 font-semibold">Largest Cluster</p>
                            </div>
                            <div className="bg-gradient-to-br from-orange-700 to-orange-800 p-6 rounded-2xl text-center shadow-lg transform hover:scale-105 transition-transform">
                                <p className="text-4xl font-bold text-white mb-2">{(totalNotes / clusters.length).toFixed(1)}</p>
                                <p className="text-sm text-orange-50 font-semibold">Avg per Cluster</p>
                            </div>
                        </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b-2 border-orange-200">
                                            <th className="text-left py-3 px-4 text-gray-700 font-semibold">Cluster</th>
                                            <th className="text-center py-3 px-4 text-gray-700 font-semibold">Notes</th>
                                            <th className="text-center py-3 px-4 text-gray-700 font-semibold">Percentage</th>
                                            <th className="text-left py-3 px-4 text-gray-700 font-semibold">Preview</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {clusters
                                            ...
                                            .map((cluster, index) => {
                                                const percentage = ((cluster.notes.length / totalNotes) * 100).toFixed(1);
                                                return (
                                                    <tr key={index} className="border-b border-gray-200 hover:bg-orange-50 transition-colors">
                                                        <td className="py-3 px-4">
                                                            <span className="font-semibold text-orange-600">{cluster.cluster_title}</span>
                                                        </td>
                                                        ...
                                                        <td className="py-3 px-4 text-center text-gray-700 font-semibold">{percentage}%</td>
                                                        <td className="py-3 px-4 text-gray-600 text-sm">
                                                            {cluster.notes[0]?.substring(0, 50)}{cluster.notes[0]?.length > 50 ? '...' : ''}
                                                        </td>
                                                    </tr>
                                                );
                                            })}
                    ...
                    <div className="text-center py-16">
                        <p className="text-xl text-gray-600">No data to display yet. Start adding notes!</p>
                    </div>
                )}
            </div>
        </div>
...
export default function NotesOrganizer() {
    ...
    // If showing stats page, render that instead
    if (showStats) {
        return <StatisticsPage clusters={clusters} onBack={() => setShowStats(false)} />;
    }
    ...
    return (
        ...
        >
            ...
            style={{ backgroundColor: 'var(--custom-app-bg)' }}>
                <div className="max-w-6xl mx-auto">
                    {/* Header */}
                    <div className="text-center mb-8">
                        ...
                        <div className="flex justify-end mb-4">
                            <button
                                ...
                                className="p-3 bg-white border-2 border-orange-300 rounded-full hover:bg-orange-50 transition-all shadow-lg group"
